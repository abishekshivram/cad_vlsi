...
def find_next(current_node, dest_node_name):
    ...
    if(current_node==None):
        print(f"Invalid node {current_node}")
        return None
...
def l1_find_next(current_node, dest_node_name):
    ...
    if (type(L1_network) != Mesh):
        print("L1 network is not mesh- invalid call to Mesh routine")
        return None
...
def is_node_in_neighbour_list(current_node,node_name):
    ...
    for neighbour in current_node.neighbour:
        if(neighbour.name==node_name):
            return neighbour
    print(f"Current node: {current_node.name} is not connected to next node name: {node_name}")
    return None
...
def get_head_node_name(current_node_name):
    '''Gets the headnode of current mesh network from name
    name is a well formatted mesh name (string
