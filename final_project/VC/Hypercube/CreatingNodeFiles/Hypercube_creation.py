...
hypercube_vc_to_core = """    rule outputLinkCore{count2}0(counter_router==2'b00);
    ...
    endrule
    rule outputLinkCore{count2}1(counter_router==2'b01);
        $display("here flit is put into core from router left vc{count2plus1}; Arbiter count-%d", counter_router);      
        Flit data_core=defaultValue;
        data_core <- router_mid.get_valueVC{count2plus1}();
        core.put_flit(data_core);
    endrule
    rule outputLinkCore{count2}2(counter_router==2'b10);
        $display("here flit is put into core from router left vc{count2plus1}; Arbiter count-%d", counter_router);      
        Flit data_core=defaultValue;
...
for address in range(8):
    ...
    with open('Hypercube_template.txt', 'r') as f:
        ...
        output_file_name = f"HypercubeNode{address}VC.bsv"
