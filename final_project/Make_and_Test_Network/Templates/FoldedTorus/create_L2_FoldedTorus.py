...
def create_L2_FT(NO_OF_NETWORKS, ALL_NETWORK_NUM, ALL_NETWORK_ID, ALL_NUMBER_OF_NODES):
    d = {}
    ...
    for i in range(NO_OF_NETWORKS):
        ...
        rule connect_Node{from_row_id}{from_col_id}_to_Node{to_row_id}{to_col_id}_U2D_dateline;
            let data{from_row_id}{from_col_id}_U2D_dateline <- node{from_row_id}{from_col_id}.get_value_to_down_dateline();
            node{to_row_id}{to_col_id}.put_value_from_up_dateline(data{from_row_id}{from_col_id}_U2D_dateline);
        endrule\n\n"""
        ...
        rule connect_Node{from_row_id}{from_col_id}_to_Node{to_row_id}{to_col_id}_D2U_dateline;
            let data{from_row_id}{from_col_id}_D2U_dateline <- node{from_row_id}{from_col_id}.get_value_to_up_dateline();
            node{to_row_id}{to_col_id}.put_value_from_down_dateline(data{from_row_id}{from_col_id}_D2U_dateline);
        endrule\n\n"""

        # NODES
        PACKAGE_NAME = "FoldedTorusNocL2_"
        ...
        for i in range(NUMBER_OF_ROWS):
            for j in range(NUMBER_OF_COLS):
                ...
                if(i==NUMBER_OF_ROWS//2 and j==NUMBER_OF_COLS//2):
                    string_node += f"\tlet node{i}{j}   <- mkFoldedTorusL2HeadNode(node{i}{j}_address, head_node_addr, {NUMBER_OF_COLS-1},{NUMBER_OF_ROWS-1});\n"
                else:
                    string_node += f"\tlet node{i}{j}   <- mkFoldedTorusNode(node{i}{j}_address, head_node_addr, {NUMBER_OF_COLS-1},{NUMBER_OF_ROWS-1});\n"
